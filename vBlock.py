...
if os.name != "nt":
    if os.path.exists("/usr/bin/vBlock") == True and os.path.exists(f"/etc/vBlock.conf") == True:
        # Parsing new path and write into handler
        sc0peConf = configparser.ConfigParser()
        sc0peConf.read(f"/etc/vBlock.conf")
        sc0pe_path = str(sc0peConf["vBlock_PATH"]["sc0pe"])
...
ARG_NAMES_TO_KWARG_OPTS = {
    ...
    "console": {"help": "Use v-block on interactive shell.", "action": "store_true"},
    ...
    "install": {"help": "Install or Uninstall v-block.", "action": "store_true"},
